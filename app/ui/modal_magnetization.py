...
class ModalMagnetization(object):
    ...
    def setupUi(self):
        ...
        self.parent.lcdNumber_dm.display(str(self.dm))
        self.parent.lcdNumber_n.display(str(self.n))
        ...
        # Tacks模型由于主要方针远点磁场，被弃用
        # self.parent.RadioButton_tamodel.clicked.connect(self.on_RadioButton_model_clicked)
        # self.parent.RadioButton_tamodel.click()
        self.parent.RadioButton_lamodel2.clicked.connect(self.on_RadioButton_model_clicked)
        self.parent.RadioButton_lamodel2.click()
        ...
        self.parent.Slider_dm.valueChanged.connect(self.on_Slider_dm_valueChanged)
        self.parent.Slider_n.valueChanged.connect(self.on_Slider_n_valueChanged)
        # 初始化figure和cavas
        ...
        for magnetization_unit in self.magnetization_unit_list:
            self.parent.ComboBoxFormLayout_step1_M_H_unit.addItem(
                magnetization_unit.get_description_with_symbol_bracket())
            self.parent.ComboBoxFormLayout_step1_M_M_unit.addItem(
                magnetization_unit.get_description_with_symbol_bracket())
    ...
    def on_RadioButton_model_clicked(self):
        if self.parent.RadioButton_jamodel.isChecked():
            ...
            self.parent.Slider_ms.setEnabled(True)
            ...
            self.parent.Slider_dm.setEnabled(False)
            self.parent.Slider_n.setEnabled(False)
            ...
            self.model = 5
            # 更新公式的图片资源
            self.parent.Label_fomula_M.setStyleSheet("image: url(:/Formula/source/formula_m_l_2.png)")
            self.parent.Slider_ms.setEnabled(False)
            self.parent.Slider_a.setEnabled(False)
            self.parent.Slider_k.setEnabled(False)
            self.parent.Slider_alfa.setEnabled(False)
            self.parent.Slider_c.setEnabled(False)
            ...
            self.parent.Slider_sigma.setValue(100)
            self.parent.Slider_dm.setEnabled(True)
            self.parent.Slider_n.setEnabled(True)
            # Tacks 是模型4
            # self.model = 4
            # # 更新公式的图片资源
    ...
    def on_Slider_dm_valueChanged(self):
        self.dm = np.power(10, np.log10(self.dm_min) + (
                np.log10(self.dm_max) - np.log10(self.dm_min)) * self.parent.Slider_dm.value() / 100.0)
        self.parent.lcdNumber_dm.display('%.1e' % self.dm)

    def on_Slider_n_valueChanged(self):
        self.n = int(np.power(10, np.log10(self.n_min) + (
                np.log10(self.n_max) - np.log10(self.n_min)) * self.parent.Slider_n.value() / 100.0))
        self.parent.lcdNumber_n.display(str(self.n))
    ...
    def on_PushButton_simulate_M_clicked(self):
        ...
        if self.model == 1:
            ...
            # sigma, d, n
            p0 = [self.sigma, self.dm, self.n]
            bounds = ([self.sigma_min, self.dm_min, self.n_min],
                      [self.sigma_max, self.dm_max, self.n_max])
        else:
            # j,sigma(OPTIONAL)
        ...
        for i in range(self.file_path.__len__()):
            ...
            if i == bg_index:
                ...
                if self.model == 1:
                    ...
                    # # FIXME: value of g_j
                    D_calc = np.power(self.g_j * sci_const.miu_B * result[1] * 6. / (np.pi * result[0]), 1 / 3)
                    if distribution_flag:
                        if self.model == 5:
                            sigma = result[0]
                            dm = result[1]
                    ...
                    item = QtGui.QStandardItem("{}, Model={},\nM_S={} A/m,\nJ={},D_calc={} m, sigma={}, Dm={} m."
                                               .format(self.file_name[i]
                                                       , {1: "Jiles_Atherton_Model", 2: "Brillouin_Model",
                                                          3: "Langevin_Model", 4: "Takacs_Model",
                                                          5: "Langevin_Model_2"}[self.model]
                                                       , result[0], result[1], D_calc
                                                       , sigma, dm))
    ...
    def on_PushButton_plot_M_clicked(self):
        ...
        for row_index in range(self.list_view_results_model.rowCount()):
            ...
            if item.isCheckable() and item.checkState() == QtCore.Qt.Checked:
                ...
                science_plot_data.add_figure_info(figure_title=data_name, x_label='External Magnetic Field,H (A/m)',
                                                  y_label='Magnetization,M (A/m)')
                science_plot_data.add_plot_data(figure_title=data_name, x_data=data_list[0].get_data(),
                                                y_data=data_list[1].get_data(), y_legend='raw')
                science_plot_data.add_plot_data(figure_title=data_name, x_data=data_list[2].get_data(),
                                                y_data=data_list[3].get_data(), y_legend='cal')
    ...
    def save_results_as(self):
        ...
        write_data = ScienceData(file_dic=file_dic, file_name=file_name, file_type=write_file_type.value,
                                 data_dict=data_dict)
