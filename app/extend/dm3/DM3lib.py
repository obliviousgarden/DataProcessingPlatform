...
class DM3(object):
    ...
    def _readTagEntry(self):
        ...
        if ( debugLevel > 5):
            print (str(self._curGroupLevel)+"|"+self._makeGroupString()+":","Tag label = "+tagLabel)
        elif ( debugLevel > 0 ):
    ...
    def _readArrayData(self, arrayTypes):
        ...
        if ( debugLevel > 3 ):
            print ("rArD, " + hex( self._f.tell() ) + ":", "Reading array of size = " + str(arraySize))
    ...
    def _storeTag(self, tagName, tagValue):
        ...
        tagName = str(tagName).replace("b\'",'').replace("\'",'')
        # - convert tag value to unicode if not already unicode object
        #   (as for string data)
        tagValue = str(tagValue).replace("b\'",'').replace("\'",'')
    ...
    def info(self):
        ...
        info_keys = {
            ...
            'pxsize': "%s.ImageData.Calibrations.Dimension.0.Scale" % tag_root,
            'pxsize_unit': "%s.ImageData.Calibrations.Dimension.0.Units" % tag_root,
